...
- 修改代码后，在当前项目的 `vibecodingdoc/yyyy-mm-dd/` 文件夹下创建一份变更说明，文件名格式为 `{修改内容}.md`，例如 `vibecodingdoc/2026-05-18/update-user-api.md`。内容应简要记录本次修改目标、涉及文件、关键变更和验证结果。文件末尾必须包含 `## 文件签名`，至少记录生成时间（格式 `yyyy-MM-dd HH:mm:ss zzz`，使用当前时区）和执行者（Codex）。
