...
- For Java backend development, participate by default alongside `java-dev` + `java-code-style` + `codegraph`. OpenSpec is still lower priority and only records/specifies intended behavior; it does not decide Java module placement, code style, constants, logging, exceptions, DTO/VO placement, DAO rules, or verification strategy.
